...
public class AjaxListCommand implements Command {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		...
		switch(reqType){
			case "xml":
				...
				break;
		/*
		 * case "json": responseJSON(request, response); break;
		 */
			default:
	...
	private void responseJSON2(HttpServletRequest request, HttpServletResponse response) {
		...
		if(dtoArr == null) {
			...
			list.setList(Arrays.asList(dtoArr));  //dtoArr[] 배열이  list로 변환
		}
		...
		try {
			...
			String jsonStirng =	mapper.writeValueAsString(list);
			
			response.setContentType("application/json; charset=utf-8");  // MINE 설정
			try {
